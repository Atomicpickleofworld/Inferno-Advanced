...
public class IABlackholeRenderer {
    private final Seq<BlackHole> holes = new Seq<>();
    private static IABlackholeRenderer renderer;
    ...
    protected IABlackholeRenderer() {
        if (!Vars.headless) {
            IABalckholeShader.createShader();
    ...
    public static void init() {
        if (renderer == null) renderer = new IABlackholeRenderer();
        for (int i = 0; i < 512; i++) {
    ...
    private void advancedDraw() {
        if (Core.settings.getBool("pixelate") || holes.size >= 512) {
            holes.clear();
            return;
        }
        ...
        Draw.draw(Layer.max - 1, () -> {
            ...
            if (holes.size >= IABalckholeShader.MaxCont) IABalckholeShader.createShader();
            ...
            IABalckholeShader.holeShader.blackHoles = blackholes;
            buffer.blit(IABalckholeShader.holeShader);
